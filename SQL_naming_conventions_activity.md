...
- Don't overuse words like records or details that don't add any context to the name.
- Don't go to one word names unless it is an overarching name when two words can allow a new viewer to have more context.
- Whatever you do, be consistent.
- When naming, be aware of what level you are on. For instance, if I am in a warehouse database naming a table `warehouse_inventory` is redundant and a name like `product_stock` would better describe a table containing product inventory.
